...
- remain the main runtime for orchestration and task execution
...
- Open WebUI on `3000`
- Agent API on `2024`
- legacy Agent Ops UI on `2024/ui`
- vLLM on `8001`
- Ollama on `11434`
- Qdrant on `6333`
- Postgres on `5432`
...
Open WebUI remains the general model chat surface. LangGraph Studio is now the primary orchestration interface for task runs and graph inspection. The FastAPI service remains mounted into the LangGraph server as custom routes for compatibility and fallback task ops.
...
- LangGraph Studio-compatible graph app with durable task and run state in Postgres
...
- built-in fallback operator dashboard at `/ui`
